 ...
 */
 /*这里修改了很多内容，主要是由于之前都是绑定的固定的 id 和 css，现在都加入了多个 case之后，只能分别标记 id 才可以正常响应*/
function StepPannel(data, pageToolId, root){
  this.pageToolId = pageToolId
  this.data = data
  ...
  this.pagesize = 20
  this.currentPage = 1
  this.stepLeftIndex = '#step-left' + pageToolId + ' .step-list'
  this.stepRightIndex = '#step-right' + pageToolId
  this.stepLeft = $(this.stepLeftIndex)
  this.stepRight = $(this.stepRightIndex)
  this.magnifyContainer = $('#magnify' + this.pageToolId +' .content')
  this.magnifyPic = $('#magnify' + this.pageToolId)
  this.scale = 0
  this.order = 'acc' // or dec
  this.duration = 'acc' // or dec
  this.status = 'acc' // or dec
  this.galleryIndex = '#gallery'+this.pageToolId
  this.galleryContent = this.galleryIndex + ' .content'
  this.orderIndex = '#order'+this.pageToolId
  this.summary = '#summary'+this.pageToolId
  ...
  this.bindEvents = function(){
    ...
    $(this.galleryContent).delegate('.thumbnail', 'click', function(e){
  ...
  this.initStepRight = function(){
    ...
    if($(".step-args .fancybox").length>0){
      fancybox = $(this.stepRightIndex+' .fancybox')
      $(this.stepRightIndex+' .fancybox .screen').load(function(e){
        // 存在截屏，并加载成功
        that.resetScale(this)
        that.convertSize($('.step-args .crop_image'), 80, 35)
        that.resetScreenshot(fancybox)
      })
    }
  }

  this.highlightBlock = function(){
    if($(this.stepRightIndex+' pre.trace').length>0){
      hljs.highlightBlock($(this.stepRightIndex+' pre.trace')[0], null, false);
    }
  }
  ...
  this.init_gallery = function(){
    ...
    if(fragment == ''){
      $(this.galleryIndex).hide()
    }else{
      $(this.galleryContent).html(fragment)
    }
  }
  ...
  this.jumpToCurrStep = function(step) {
    // 跳至指定步骤
    step = step || (this.steps.length > 0 ? this.steps[0].index : 0)
    step = step - this.pageToolId*1000
  ...
  this.showMagnifyPic = function(fragment) {
    ...
    if (fancybox.length > 0){
      var that = this
      $('#magnify' + this.pageToolId +' .fancybox .screen').load(function(e){
        // 存在截屏，并加载成功
        if (this.height > this.parentNode.offsetHeight){
          this.style.height = this.parentNode.offsetHeight + 'px'
        }
        that.resetScale(this)
        that.resetScreenshot($('#magnify' + this.pageToolId +' .fancybox'))
      })
    }
...
//      console.log(step.index)
  ...
  this.setStepRight = function(index){
    ...
    if(!isNaN(index) && index>= 0 && index<this.original_steps.length){
      $(this.galleryIndex + ' .thumbnail.active').removeClass('active')
      $(this.galleryIndex + ' .thumbnail[index="%s"]'.format(index)).addClass('active')
  ...
  pageToolAndIndex = '#pageTool' + pageToolId
  this.init_pagenation = function(){
    ...
    $(pageToolAndIndex).prepend('<span class="steps-total"><span lang="en">Total </span><span class="steps-account"></span></span>')
  }

  this.setPagenation = function(){
    if(this.steps.length > this.pagesize)
      $(pageToolAndIndex).show()
    else
      $(pageToolAndIndex).hide()
    this.paging.render({
      'count': this.steps.length
    })
    $(pageToolAndIndex + ' .steps-account').html(this.steps.length)
...
function loadUrlInfo(data){
  ...
  set_task_status(result, this.summary, data)
...
function set_task_status(result, summary, data){
...
function init_page(summary, data){
 console.log($(summary+' .info-sub.start'))
...
function init_steps(data, pageToolId){
  summary = '#summary'+pageToolId
  init_page(summary, data)
  stepPanel = new StepPannel(data, pageToolId)
  stepPanel.init()
  $("img").error(function () {
  ...
  // 自动收缩过长的脚本描述
  var descHeight = 100;
  descWrap = $(this.summary+' .airdesc')
  if($(this.summary+' .desc-content').height()>descHeight) {
    descWrap.addClass('long collapse')
    $(this.summary+' .show-more').click(function(){
...
function GenerateCPUMEMORY(cpu_memory_id, cpu_memory){
    var myChart = echarts.init(document.getElementById('cpu_memory'+cpu_memory_id), darkTheme);
    option = {
        title : {
            text: 'CPU Memory',
        },
